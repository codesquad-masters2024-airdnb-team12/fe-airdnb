...
<script>
  ...
  const handleMinChange = (event) => {
    selectedMinPrice = roundToNearestThousand(
        Math.min(event.target.value, selectedMaxPrice - 1000));
  };

  const handleMaxChange = (event) => {
    selectedMaxPrice = roundToNearestThousand(
        Math.max(event.target.value, selectedMinPrice + 1000));
  };
...
<div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center text-black">
  <div class="popup bg-white p-6 rounded-lg shadow-lg">
    <div class="range-slider">
      ...
      <div class="slider">
        <input id="min-price" type="range" min={minPrice} max={maxPrice}
               bind:value={selectedMinPrice} on:input={handleMinChange} step="1000"/>
        <input id="max-price" type="range" min={minPrice} max={maxPrice}
               bind:value={selectedMaxPrice} on:input={handleMaxChange} step="1000"/>
      </div>
    </div>
